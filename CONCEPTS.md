...
without needing knowledge of previous or future compression stages.
...
Compression techniques inherit from
...
keeps every downstream component synchronized automatically.

This became particularly important for

- LoRA
- Distillation

because both produce new model instances.

Whenever a technique replaces the model, it must also update

trainer.model

to avoid training stale parameters.
...
and replaces the LoRA module with a standard Linear layer.
...
The implementation recreates the optimizer using

type(trainer.optimizer)

and copies only compatible defaults.

One important issue encountered was

AdamW

containing internal fields such as

decoupled_weight_decay

that are not accepted by the constructor.
...
unsupported arguments are filtered before reconstruction.
...
latency increased substantially on Apple Silicon.
...
contains millions of trainable parameters.
...
so no additional computation remains.
...
Gradients then flow through both
...
If both matrices were initialized randomly,

the model output would immediately change,

causing a large accuracy drop before training even begins.

If both matrices were initialized to zero,

the gradients would remain symmetric,
...
1.
The update magnitude remains approximately independent of the chosen rank.

2.
Changing rank does not require retuning learning rates.

3.
The optimizer behaves consistently for different LoRA configurations.
...
Memory usage decreases by approximately
...
To determine suitable activation ranges,

the model is run on representative data.
...
Embedding weights require

float_qparams_weight_only_qconfig

which uses floating-point zero points.

Without this qconfig,

embedding quantization fails or produces poor accuracy.

Therefore every Embedding module is assigned a custom qconfig before calling
...
which is highly sensitive to numerical precision.
...
quantizing LayerNorm produced runtime failures on Apple Silicon.

Therefore

LayerNorm.qconfig = None

allowing LayerNorm to remain floating point while Linear layers were quantized.
...
Higher temperatures soften the probability distribution,

revealing similarities between classes.
